...
struct Qcircuit {
    ...
    Qcircuit(int nQ, int nC = 0): n_qbits(nQ), n_cbits(nC),order_ptr(0), c_reg(0) {
        std::vector < struct state_vector > sv;
        sv.resize(nQ);
        for(int i = 0; i < nQ; i++) ketZero(sv[i]);
        if(nQ > 1)
    ...
    Qcircuit(std::string path): order_ptr(0), c_reg(0) {
        ...
        sv.resize(n_qbits);
        for(int i = 0; i < n_qbits; i++) ketZero(sv[i]);
        if(n_qbits > 1)
            for(int i = 0; i < n_qbits - 1; i++) sv[i + 1] = sv[i] * sv[i + 1];
        ZERO = state = sv[n_qbits - 1];
        //ORDER.push_back({4,true}); // false means hard reset 
    }
    void applyGate(const std::string & gate, int t_qbit) {
        int GATE;
        if(gate == "H" || gate == "h") GATE = 1;
        else if(gate == "x" || gate == "X") GATE = 2;
        else if(gate == "z" || gate == "Z") GATE = 4;
        else if(gate == "y" || gate == "Y") GATE = 3;
    ...
    void applyControlledGate(const std::string & gate,
        ...
        iss >> n;
        int third = (cq == 'q') ? 1 : 0;
        if(gate == "H" || gate == "h") ORDER.push_back({ 2, 1, third, n, t_qbit });
        else if(gate == "x" || gate == "X") ORDER.push_back({ 2, 2, third, n, t_qbit });
        else if(gate == "z" || gate == "Z") ORDER.push_back({ 2, 4, third, n, t_qbit });
    }
    ...
    void uploadCircuit(const std::vector < std::vector < int >> & ORDER,
        ...
        else std::cerr << "Error: Unable to open file " << full << "\n";
    }
    std::vector < std::vector < int >> downloadCircuit(const std::string & path) {
        ...
        if(infile.is_open()) {
            ...
            while(ordersize--) {
                ...
                while(iss >> val) inner.push_back(val);
        ...
        else std::cerr << "Error: Unable to open file " << path << ".txt\n";
        return result;
    }
...
struct result Qcircuit::simulate(int shots = 1, std::string path = "", bool describe = false) {
        if(describe) std::cout << "simulation starting with shots: " << shots << std::endl;
        if(path != "") {
            if(describe) std::cout << "uploading circuit to " << path << std::endl;
            ...
            for(std::vector<std::vector<int>>::size_type i = 0; i < ORDER.size(); i++) {
                ...
                else if(ORDER[i][0] == 1) // applygate 
                {
                    ...
                    for(int ii = 0; ii < n; ++ii) {
                        for(int j = 0; j < n; ++j) result[ii] = result[ii] + (buffedmatrix[ii][j] * state.coeffs[j]);
                    }
                    state.coeffs = result;
                }
                else if(ORDER[i][0] == 2) // controlledgate 
                {
                    ...
                    if(ORDER[i][2] == 1) // type: q
                    {
                        ...
                        else if(ORDER[i][1] == 2) GATES::controlled_pauli_X(state, ORDER[i][3], ORDER[i][4]);
                ...
                else if(ORDER[i][0] == 3) // measure 
                {
                    ...
                    double prob = 0.0;
                    for(int i = 0; i < cnd; i++)
                        if(i & tmask) prob += state.coeffs[i].amplitude();
                    ...
                    if(res) c_reg = c_reg | (1 << (n_cbits - 1 - ORDER[i][2]));
                ...
                else if(ORDER[i][0] == 4) // clear 
                {
                    ...
                    if(ORDER[i][1] == 0) //hard reset
                    {
                        ...
                        state.n_qbits = 0;
                        state.coeffs.clear();
                        for(auto & inner_vector: ORDER) inner_vector.clear();
                        ORDER.clear();
                    }
                    else // back to ZERO 
                    {
                ...
                else if(ORDER[i][0] == 10) //set c bit 
                {
                    ...
                    if(ORDER[i][1] == 1) c_reg = c_reg | (1 << (n_cbits - 1 - ORDER[i][2]));
                    else if(ORDER[i][1] == 2) c_reg = c_reg & (~(1 << (n_cbits - 1 - ORDER[i][2])));
                    if(describe) std::cout << "currently c_reg: " << c_reg << std::endl;
                }
            }
            m[c_reg]++;
        }
